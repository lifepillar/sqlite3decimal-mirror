 ...
 * extracted out of the `manifest`, `manifest.uuid`, and `Makefile` files. Call
 * this program with three arguments:
 *
 *     ./a.out manifest.uuid manifest Makefile
 *
 * Note that the `manifest.uuid` and `manifest` files are generated by Fossil.
 *
 * \note Adapted by Lifepillar from Fossil's mkversion.c to read the version
 *       string from the Makefile rather than from a `VERSION` file.
 */
#include <ctype.h>
#include <stdio.h>
#include <string.h>
...
int main(int argc, char *argv[]){
  (void)argc;
    ...
    *z = 0;
    ...
    int not_found = 1;
    char line[1000];
    while (not_found && fgets(line,sizeof(line)-1,v)){
    ...
    if (not_found) {
        fprintf(stderr, "malformed VERSION field: %s\n", argv[3]);
        exit(1);
    }
    fclose(v);
    printf("\n/**\n");
